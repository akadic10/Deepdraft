...
LEAF_TOP  = _c(0x7CB342)
LEAF_MID  = _c(0x558B2F)
LEAF_DARK = _c(0x33691E)
SPR_TOP   = _c(0xAED581)
SPR_MID   = _c(0x8BC34A)
BLOSSOM   = [_c(0xF8D7E6), _c(0xFFFFFF), _c(0xF4B6C8)]
AU_GOLD   = _c(0xE0A526)
AU_RUST   = _c(0xB5561E)
AU_DARK   = _c(0x7A3B16)
APPLE     = [_c(0xD7263D), _c(0xE8412B), _c(0xC11E33)]
TR_HI     = _c(0x9A5E38)
TR_MID    = _c(0x7A4A2B)
TR_DK     = _c(0x542F18)
BRANCH    = _c(0x5A3920)
...
def add_canopy(v, cy, rx, ry, rz, season, rng):
    ...
    if season == "spring":
        for (x, y, z), c in list(v.cells.items()):
            if (x, y + 1, z) not in v.cells and y > cy - ry * 0.3 and rng.random() < 0.14:
                v.cells[(x, y, z)] = vary(rng.choice(BLOSSOM), 0.04, rng)
    if fruiting:
        for (x, y, z), c in list(v.cells.items()):
            below_open = (x, y - 1, z) not in v.cells
            outer = abs(x) > rx * 0.4 or abs(z) > rz * 0.4
            if y < cy + ry * 0.4 and outer and below_open and rng.random() < 0.10:
                v.cells[(x, y, z)] = vary(rng.choice(APPLE), 0.05, rng)


STAGES = {
    "sapling": dict(H=6,  cy=4,  rx=2, ry=2, trunk_w=1, bare=2, flare=0),
    "mature":  dict(H=13, cy=9,  rx=5, ry=4, trunk_w=2, bare=4, flare=1),
    "ancient": dict(H=19, cy=13, rx=8, ry=6, trunk_w=3, bare=5, flare=2),
}
...
def build_apple(stage, season="summer", variant=1):
    ...
    rz = max(1, int(round(p["rx"] * VMUL[variant])))
    ry = p["ry"]
    cy = p["cy"]
