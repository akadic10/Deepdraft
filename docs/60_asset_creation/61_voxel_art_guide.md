...
Footprint: **1×1**, 2 blocks tall, **wall-adjacent** (solid back panel against the wall).
Collision: `[{min:[0,0,0], max:[1,2,1]}]`.

Heavy oak corner uprights (`wood_oak_mid`), base plinth + mid shelf + top cap in planks
(`wood_plank`, `wood_plank_dark` edges), solid plank back panel (`wood_oak_dark`), iron
brackets (`iron_dull`) under each level against the posts. Wood per the 2026-07-11 material
rule — furniture is woodwork. **Contents render on the shelf**: stored item GLBs sit on the
8 anchor points (4 per level, defined in `data/furniture/storage_shelf.json`) at ~0.5 scale
— capacity equals anchor count, WYSIWYG. Generated by `tools/generate_furniture_glbs.py`;
