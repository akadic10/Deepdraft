...
| Block Inspector Agreement        | Done        | Inspector now reports render mode, hit source, face, hit/generated block keys, agreement status, domain, domain value, surface/visible Y, visible top block, water/bank flags, kind, color, and coordinates. Use the debug window for manual spot-click verification.      |
| Surface Diorama Decision         | Done        | The misleading top-only surface view is quarantined as historical context. Runtime code now names the active high-altitude path `block-face overview`, labels it as an approximation in debug UI/inspector text, and keeps top/side faces derived from sampled world data. |
| Block-Faithful Overview Renderer | Done        | Overview emits block-derived top faces and side faces with deterministic vertical material approximation. Same-height, same-material top faces are greedily merged, sampled top IDs validate against generated blocks, and current side faces read cleanly in debug review. |
