...
4. **"Two dwarves idle while a door ghost waits; a third eventually installed it"
   (Alen, 2026-08-07 screenshots) — the HAUL/FETCH_BUILD item race.** Task priority
   (FETCH_BUILD 45 > HAUL 40) orders *assignment*, but the physical item had none: a
   stockpile hauler pouched the matching packed item (packed furniture carries
   `stockpile_furniture`, so zones restock it like any drop), and while it rode the pouch
   `item_available()` was false → the ghost had **no lease at all** → idle dwarves had
   nothing to take. Only the deposit's `stockpile_changed` wake posted the lease, and the
   fetcher then walked to the zone and back. → **Fixed same day: ghost item CLAIMS** (SH
   restock parity — an item pending placement is never restocked). On every lease wake the
   ghost claims the nearest matching loose item (an ItemDropManager reservation under
   `claim_owner_id()` = 1e9 + source_id, positive so the unreserve owner-guard stays
   effective); every hauler scan is unreserved-only, so claimed items are invisible to
   restock. `reserve_fetch` hands the claim to the fetching dwarf (claim-first — a claimed
   item is reserved, so the unreserved scan would never find it); `cancel_fetch` re-claims
   on release; the controller releases claims on ghost teardown. Residual, accepted: an
   item **already inside a pouch** when the ghost is placed stays unavailable until the
   hauler deposits or drops it — stealing from pouches is out of scope.

| Date | Steps | State |
|---|---|---|
| 2026-08-07 | **Item-claim pass (playtest note 4 — the HAUL/FETCH_BUILD item race).** `FurnitureGhostComponent` gains `_claim` + `claim_owner_id()` (1e9 + source_id) + `_ensure_claim()` (wake-driven, from `update_lease`) + `release_claim()`; `item_available()` counts a live claim; `reserve_fetch` transfers the claim to the dwarf before falling back to loose-scan/storage-withdraw; `cancel_fetch` re-claims the freed item. Controller: `cancel_ghost` and `_on_ghost_build_complete` release claims. Claims are runtime-only, never saved — they rebuild from the restore-path wakes (`restore_loose_item` emits `drop_spawned`). No new class_name, no autoload change — plain hot-reload. | Implemented — verify in-engine: place a ghost beside loose packed items while zone hauling runs; the fetch lease should post immediately, an idle dwarf should take it (never re-routed via the stockpile), and haulers should visibly skip the claimed box. |
| 2026-07-11 | **Phase 5 (shelf contents on the anchors — the last milestone piece).** `ContainerStorageComponent` grew the anchor renderer: `render_contents` pieces snap each deposited node onto the next free anchor as a CHILD of the piece's visual node (footprint-local offsets from `storage_shelf.json`, re-centred to the node origin) at `anchor_scale` 0.5 with deterministic per-anchor yaw variation (SH ATTITEM parity) — the piece's own rotation and slice-culling apply for free. Withdraws visibly remove an anchored item (`_pop_anchor_node`) before the pre-reserved drop spawns at the stand cell; uninstall dump clears the anchors then respawns counts as loose drops. Barrel/chest keep absorbing. Controller hands `display_parent` at install. | **VERIFIED in-engine by Alen, 2026-07-11 ("Seems to be working fine" — screenshot: items visibly stocked on both shelf levels). Phase 5 BANKED — MILESTONE COMPLETE.** Alen's hand-authored shelf model still slots in whenever ready (anchors are data, not mesh). |
